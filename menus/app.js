...
function formatSchedule(restaurant) {
  ...
  const rows = [1, 2, 3, 4, 5, 6, 0].map(day => {
    ...
    if (d.closed) return `<div class="schedule-row"><span class="schedule-day">${name}</span><span class="schedule-closed">${closed}</span></div>`;
    const parts = [];
...
function renderActions(r) {
  ...
  const btns = [];
  if (r.phone)
    btns.push(`<a href="tel:${r.phone}" class="btn btn-primary">${SVG.phone} ${t('call', lang)}</a>`);
  if (r.whatsapp)
    btns.push(`<a href="https://wa.me/${r.whatsapp.replace(/\D/g,'')}" class="btn btn-whatsapp" target="_blank">${SVG.whatsapp} ${t('whatsapp', lang)}</a>`);
  if (r.mapsUrl)
    btns.push(`<a href="${r.mapsUrl}" class="btn btn-maps" target="_blank">${SVG.maps} ${t('directions', lang)}</a>`);
